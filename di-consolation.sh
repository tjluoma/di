...
then
	INSTALL_TO='/Volumes/Applications/Consolation3.app'
else
	INSTALL_TO='/Applications/Consolation3.app'
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' Consolation$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' Consolation$' | awk '{print $NF}' | tail -1)
...
FILENAME="$HOME/Downloads/Consolation-${LATEST_VERSION}.zip"

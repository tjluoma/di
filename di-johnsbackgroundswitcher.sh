#!/usr/bin/env zsh -f
# Purpose: Download and install/update the latest version of John's Background Switcher https://johnsad.ventures/software/backgroundswitcher/
#
...
INFO=($(curl -sfLS "$XML_FEED" | awk '/<item>/{i++}i==1' | tr ' ' '\012' | sort | egrep -i '(url|sparkle:version)=' | awk -F'"' '//{print $2}'))
...
FILENAME="$HOME/Downloads/JohnsBackgroundSwitcher-${${LATEST_VERSION}// /}.zip"

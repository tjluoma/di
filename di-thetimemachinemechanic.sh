...
then
	INSTALL_TO='/Volumes/Applications/TheTimeMachineMechanic.app'
	TRASH="/Volumes/Applications/.Trashes/$UID"
else
	INSTALL_TO='/Applications/TheTimeMachineMechanic.app'
	TRASH="/.Trashes/$UID"
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' T2M2$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' T2M2$' | awk '{print $NF}' | tail -1)
...
echo "$NAME: Moving new version of '$INSTALL_TO:t' (from '$UNZIP_TO') to '$INSTALL_TO'."
	...
	## Note the extra '/*/' which is something most eclecticapps do

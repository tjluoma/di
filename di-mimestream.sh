#!/usr/bin/env zsh -f
# Purpose:	Download and install the latest version of MIMEStream
#
...
INFO=$(curl -sfLS "$XML_FEED" \
		| awk '/<item>/{i++}i==1' \
		| sed -e 's#^ *##g' -e 's#\<!\[CDATA\[##g' -e 's#\]\]>##g' )

DETAILS=($(echo "$INFO" | tr ' ' '\n' | egrep 'url|sparkle:version|sparkle:shortVersionString' | sort | awk -F'"' '//{print $2}'))

LATEST_VERSION="$DETAILS[1]"

LATEST_BUILD="$DETAILS[2]"

...
then
	INSTALL_TO='/Volumes/Applications/LockRattler.app'
	TRASH="/Volumes/Applications/.Trashes/$UID"
else
	INSTALL_TO='/Applications/LockRattler.app'
	TRASH="/.Trashes/$UID"
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' LockRattler$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' LockRattler$' | awk '{print $NF}' | tail -1)

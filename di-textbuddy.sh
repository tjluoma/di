...
RELEASE_NOTES_AS_HTML=$(echo "$INFO" | sed '1,/<description>/d; /<\/description>/,$d')

URL=$(echo "$INFO" | fgrep '<enclosure ' | sed -e 's#\.zip.*#.zip#g' -e 's#.*http#http#g')
...
then
	...
	then
		...
		RELEASE_NOTES=$(echo "$RELEASE_NOTES_AS_HTML" | lynx -dump -width='10000' -display_charset=UTF-8 -assume_charset=UTF-8 -pseudo_inlines -stdin -nomargins)
		...
		echo "$RELEASE_NOTES_AS_HTML" > "$RELEASE_NOTES_HTML"

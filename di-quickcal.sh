...
then
	INSTALL_TO='/Volumes/Applications/QuickCal.app'
	TRASH="/Volumes/Applications/.Trashes/$UID"
else
	INSTALL_TO='/Applications/QuickCal.app'
	TRASH="/.Trashes/$UID"
fi
...
XML_FEED='http://quickcalapp.com/app/appcast.xml'
...
INFO=($(curl -sfLS "$XML_FEED" \
		...
		| head -4 \
		| sort \
		| sed 's#</sparkle:releaseNotesLink>##g ; s#<sparkle:releaseNotesLink>##g ; s#"$##g ; s#.*"##g'))
...
then
	...
	lynx -dump -nomargins -width='100' -assume_charset=UTF-8 -pseudo_inlines "$RELEASE_NOTES_URL" | tee "$FILENAME:r.txt"

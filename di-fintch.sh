#!/usr/bin/env zsh -f
# Purpose: Dintch – check the integrity of your files
#
...
INSTALL_TO='/Applications/Fintch.app'
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' Fintch$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' Fintch$' | awk '{print $NF}' | tail -1)

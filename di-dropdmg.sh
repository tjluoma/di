...
then
	INSTALL_TO='/Volumes/Applications/DropDMG.app'
else
	INSTALL_TO='/Applications/DropDMG.app'
fi
...
LATEST_VERSION=$(sed '1,/<key>com.c-command.DropDMG<\/key>/d; /<\/dict>/,$d' "$TEMPFILE" \
				| awk '/\<key\>Version\<\/key\>/{getline ; print}'  \
				| sed 's#.*<string>##g ; s#</string>##g')

URL=$(sed '1,/<key>com.c-command.DropDMG<\/key>/d; /<\/dict>/,$d' "$TEMPFILE" \
...
then
	...
	RELEASE_NOTES=$(sed '1,/<key>com.c-command.DropDMG<\/key>/d; /<\/dict>/,$d' "$TEMPFILE" \

#!/usr/bin/env zsh -f
# Purpose: Taccy – troubleshoot privacy problems
#
...
then
	INSTALL_TO='/Volumes/Applications/Taccy.app'
	TRASH="/Volumes/Applications/.Trashes/$UID"
else
	INSTALL_TO='/Applications/Taccy.app'
	TRASH="/.Trashes/$UID"
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' Taccy$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' Taccy$' | awk '{print $NF}' | tail -1)
...
echo "$NAME: Moving new version of '$INSTALL_TO:t' (from '$UNZIP_TO') to '$INSTALL_TO'."
	...
	# Move the file out of the folder -- note the difference from usual

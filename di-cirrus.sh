...
then
	INSTALL_TO='/Volumes/Applications/Cirrus.app'
else
	INSTALL_TO='/Applications/Cirrus.app'
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' Cirrus$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' Cirrus$' | awk '{print $NF}' | tail -1)
...
FILENAME="$HOME/Downloads/Cirrus-${LATEST_VERSION}.zip"

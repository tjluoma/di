...
XML_FEED='https://version.dssw.co.uk/powermanager/application'

TEMPFILE="${TMPDIR-/tmp}/${NAME}.$$.$RANDOM.xml"

curl -sfLS "$XML_FEED" >| "$TEMPFILE"

LATEST_VERSION=$(tr ' ' '\012' < "$TEMPFILE" | awk -F'"' '/^sparkle:shortVersionString/{print $2}')

LATEST_BUILD=$(tr ' ' '\012' < "$TEMPFILE" | awk -F'"' '/^sparkle:version/{print $2}')

	# This always seems to be
	#	 URL='https://www.dssw.co.uk/powermanager/dsswpowermanager.dmg'
	# but there's no real harm in making sure
...
then
	...
	RELEASE_NOTES=$(tr '\012' ' ' < "$TEMPFILE" | sed 's#.*\[CDATA\[##g; s#\]\].*##g' | lynx -dump -nomargins -width='10000' -assume_charset=UTF-8 -pseudo_inlines -stdin)

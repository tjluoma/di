...
then
	INSTALL_TO='/Volumes/Applications/Signet.app'
	TRASH="/Volumes/Applications/.Trashes/$UID"
else
	INSTALL_TO='/Applications/Signet.app'
	TRASH="/.Trashes/$UID"
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -i -B1 ' signet$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -i -A1 ' signet$' | awk '{print $NF}' | tail -1)

#!/bin/zsh -f
# Purpose: 32-bitCheck https://eclecticlight.co
#
...
then
	INSTALL_TO='/Volumes/Applications/32-bitCheck.app'
else
	INSTALL_TO='/Applications/32-bitCheck.app'
fi
...
LATEST_VERSION=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -B1 -i ' 32-bitCheck$' | awk '{print $NF}' | head -1)

URL=$(/usr/libexec/PlistBuddy -c print "$TEMPFILE" | egrep -A1 -i ' 32-bitCheck$' | awk '{print $NF}' | tail -1)
